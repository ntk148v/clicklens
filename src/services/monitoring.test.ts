...
describe("MonitoringService", () => {
  ...
  beforeEach(() => {
    mockClient = {
      query: mock((query: string) => {
        ...
        if (query.includes("system.metrics")) { // Overview query approximation
             if (query.includes("uptime")) return Promise.resolve({ data: mockOverview });
        }
  ...
  it("should fetch dashboard data successfully", async () => {
    ...
    mockClient.query = mock((query: string) => {
        if (query.includes("SELECT DISTINCT cluster FROM system.clusters")) return Promise.resolve({ data: mockClusters });
        if (query.includes("count() AS total_nodes")) return Promise.resolve({ data: mockClusterSummary });
        if (query.includes("SELECT\n  cluster,\n  shard_num")) return Promise.resolve({ data: mockClusterNodes });
        if (query.includes("(SELECT version()) AS version")) return Promise.resolve({ data: mockOverview });
