...
ClickLens can connect to multiple independent ClickHouse clusters from a single instance.

### 5.1. Configuration

Instead of `CLICKHOUSE_HOST`/`LENS_USER`, set `CLICKHOUSE_CLUSTERS`:

```bash
# Deprecated: CLICKHOUSE_HOST/LENS_USER — migrate to CLICKHOUSE_CLUSTERS below
# CLICKHOUSE_HOST=clickhouse.example.com
# LENS_USER=lensuser
# LENS_PASSWORD=password

# Canonical: CLICKHOUSE_CLUSTERS (single or multi cluster)
CLICKHOUSE_CLUSTERS='[
