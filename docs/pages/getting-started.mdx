...
> **Legacy:** `CLICKHOUSE_HOST`/`LENS_USER` are deprecated — see the [deployment guide](/deployment) for migration.
...
services:
  app:
    ...
    ports:
      - "3000:3000"
    environment:
      - CLICKHOUSE_CLUSTERS=[{"id":"primary","label":"Primary","host":"your-clickhouse-host","port":8123,"lensUser":"lensuser","lensPassword":"your-password"}]
...
### 3.1. Lens Service User (Cluster Registry Credentials)

Configured via `lensUser`/`lensPassword` in each `CLICKHOUSE_CLUSTERS` entry (or the deprecated `LENS_USER`/`LENS_PASSWORD` environment variables for single-cluster setups). This service account is used for:
...
GRANT REMOTE ON *.* TO lensuser;

-- Cluster
CREATE USER lensuser ON CLUSTER your_cluster_name IDENTIFIED BY 'password';
GRANT ACCESS MANAGEMENT ON *.* ON CLUSTER your_cluster_name TO lensuser;
GRANT SELECT ON system.* ON CLUSTER your_cluster_name TO lensuser;
GRANT SHOW DATABASES ON *.* ON CLUSTER your_cluster_name TO lensuser;
GRANT SHOW TABLES ON *.* ON CLUSTER your_cluster_name TO lensuser;
GRANT SHOW COLUMNS ON *.* ON CLUSTER your_cluster_name TO lensuser;
GRANT REMOTE ON *.* ON CLUSTER your_cluster_name TO lensuser;
```
...
<Callout type="info">
  Redis is completely optional. If `REDIS_URL` is not configured, ClickLens will
  function normally without caching. Redis is recommended for production
  deployments with multiple users or frequent monitoring usage.
</Callout>

## 5. Multi-Cluster Support

ClickLens can connect to multiple independent ClickHouse clusters from a single instance. Set `CLICKHOUSE_CLUSTERS` with one entry per cluster:

```bash
# Single cluster:
CLICKHOUSE_CLUSTERS='[
  {"id":"primary","label":"Primary","host":"clickhouse.example.com","port":8123,"lensUser":"lensuser","lensPassword":"password"}
]'
...
> **Note:** The deprecated `CLICKHOUSE_HOST`/`LENS_USER` env vars are still supported when `CLICKHOUSE_CLUSTERS` is not set. See the [deployment guide](/deployment) for migration.

### 5.2. Multi-Cluster Login

The login page shows a cluster selector. Select a cluster, enter your ClickHouse credentials for that cluster, and log in. Every subsequent operation (queries, monitoring, schema browsing, access control) targets that cluster.

### 5.3. Docker Compose (Multi-Cluster)

A standalone compose file with two independent ClickHouse nodes is provided:

```bash
# Start both clusters and ClickLens
docker compose -f docker-compose.multi-cluster.yml up --build -d

# View init logs (credential setup)
docker compose logs clickhouse-alpha
```

Each node runs its own init script (`dev/multi-cluster/init-*.sh`) that creates demo users. After changing credentials, wipe volumes so scripts re-run:
...
2. Select a cluster from the dropdown (if multi-cluster is configured)
...
1. Verify `CLICKHOUSE_CLUSTERS` host and port values are correct
...
<Callout type="info">
  Redis is optional. If Redis is unavailable, ClickLens will continue to
  function normally without caching. You can temporarily disable Redis by
  removing the `REDIS_URL` environment variable.
</Callout>
