...
- **Dual Client Model**: Separates privileged operations (via `LENS_USER` / per-cluster `lensUser`) from user-context queries (via session credentials)
- **Multi-Cluster Support**: Each login session is pinned to one registered cluster; caches and metadata lookups are isolated by `config.clusterId`
...
└── lib/                    # Core business logic (31 files)
    ...
    ├── clickhouse/         # ClickHouse client, config, **cluster registry**
...
- **Credentials**: From environment variables (`LENS_USER`, `LENS_PASSWORD`) or per-cluster `lensUser`/`lensPassword` in `CLICKHOUSE_CLUSTERS`
...
- **Multi-Cluster**: Each registered cluster has its own Lens credentials; the correct one is selected based on the session's `clusterId`
...
interface SessionData {
  isLoggedIn: boolean;
  sessionId?: string;
  clusterId?: string; // Cluster ID from CLICKHOUSE_CLUSTERS registry
  user?: {
    username: string;
...
- `/api/auth/clusters` - List available clusters (multi-cluster mode, public endpoint)
...
- `CLICKHOUSE_CLUSTERS` - JSON array of cluster definitions. Each entry requires `id`, `label`, `host`, `lensUser`, `lensPassword`. One entry = single cluster, more = multi-cluster.
- `SESSION_SECRET` - Session encryption key (min 32 chars)

**Deprecated (migrate to `CLICKHOUSE_CLUSTERS` above)**:

- `CLICKHOUSE_HOST` - ClickHouse server address (migrate to `host` in `CLICKHOUSE_CLUSTERS`)
- `CLICKHOUSE_PORT` - Default: `8123` (migrate to `port` per registry entry)
- `CLICKHOUSE_SECURE` - Default: `false` (migrate to `secure` per registry entry)
- `CLICKHOUSE_VERIFY` - Default: `true` (migrate to `verifySsl` per registry entry)
- `LENS_USER` - Service account username (migrate to `lensUser` in `CLICKHOUSE_CLUSTERS`)
- `LENS_PASSWORD` - Service account password (migrate to `lensPassword` in `CLICKHOUSE_CLUSTERS`)
...
| ------------------ | --------------------------------------------------------- | ---------- |
| Table schema       | `clicklens:schema:{clusterId}:{database}:{table}`         | 5 minutes  |
| Table parts        | `clicklens:parts:{clusterId}:{database}:{table}`          | 1 minute   |
| Monitoring metrics | `clicklens:metrics:{clusterId}:{metric_name}`             | 30 seconds |
| Query analytics    | `clicklens:analytics:{clusterId}:{time_range}`            | 1 minute   |
| Time-series data   | `clicklens:timeseries:{clusterId}:{metric}:{start}:{end}` | 2 minutes  |

All cache keys include `{clusterId}` to isolate data between clusters when multi-cluster mode is active. Falls back to `"legacy"` for single-cluster deploys.
...
<Callout type="info">
  Redis caching is completely optional. ClickLens functions normally without
  Redis, but enabling it significantly improves performance for monitoring and
  table browsing features.
</Callout>
...
**Lens Service Account Setup** (defined via `lensUser`/`lensPassword` in `CLICKHOUSE_CLUSTERS`, or the deprecated `LENS_USER`/`LENS_PASSWORD` env vars):
