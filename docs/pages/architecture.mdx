...
**Fallback Probes for XML Users:**

If `system.grants` returns no privileges (which occurs for users configured via `users.xml`), ClickLens falls back to user-credential probes:

- Uses `SHOW GRANTS FOR [user]` to scrape actual access
- Probes tables like `system.tables` or `system.databases` using the credentials directly to verify access
...
ClickLens implements several robust security measures at the architectural level:

- **Global Authentication Middleware**: Next.js middleware enforces strict authentication checks across all internal and API routes.
- **Advanced Rate Limiting**: Built-in rate limiting with sliding window tracking to prevent brute force attacks, utilizing `x-forwarded-for` to mitigate IP spoofing via proxies.
- **Encrypted Server-Side Sessions**: Using `iron-session` with AES-256-GCM encryption ensures database credentials stay securely on the backend, shielded from Cross-Site Scripting (XSS).
- **SQL Injection Defenses**: The ClickHouse client handles comprehensive escaping and null-byte stripping to neutralize malformed SQL injection vectors.
- **Content Security Policy (CSP)**: ClickLens applies modern CSP HTTP headers to prevent XSS attacks and frame injections natively in browsers.
