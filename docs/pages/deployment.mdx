...
| ---------------------------- | -------------------------------------------------------------------------------------------------- | ------------------------------------------------------ |
| `CLICKHOUSE_CLUSTERS`        | JSON array of cluster definitions. One entry = single cluster, more = multi.                       | `[...]` (see example below)                            |
...
These legacy environment variables still work when `CLICKHOUSE_CLUSTERS` is not set. A deprecation warning is printed once at startup. They will be removed in the next major release.

| Variable             | Maps to in `CLICKHOUSE_CLUSTERS` |
| -------------------- | -------------------------------- |
...
If both `CLICKHOUSE_CLUSTERS` and legacy vars are set, the registry takes precedence.

### 1.4. Migration from Legacy Env Vars

If you're upgrading from the legacy `CLICKHOUSE_HOST`/`LENS_USER` config:

1. Create a `CLICKHOUSE_CLUSTERS` JSON array with one entry for your cluster (see §2.4 sample)
2. Remove the legacy vars from your env (optional — they still work, just deprecated with a `console.warn`)
3. Restart — login, queries, monitoring all work identically

<Callout type="info">
  **Sub-path (`CLICKLENS_BASE_PATH`)** is a **build-time** setting. Set it in
  the same environment you use for `next build`, your Docker `docker build`, or
  your CI image build. It is compiled into the client as `NEXT_PUBLIC_BASE_PATH`
  so the browser can load `/api/…` routes and static assets under the same
  prefix. Your reverse proxy (or Kubernetes ingress) path must **match** this
  value (e.g. both use `/clicklens`). The production sample below keeps the app
  at the site root; for a sub-path, see **§5.2** below.
</Callout>
...
```bash
# ClickHouse Cluster Configuration (required)
# One entry = single cluster, more entries = multi-cluster
CLICKHOUSE_CLUSTERS='[
  {"id":"primary","label":"Primary","host":"localhost","port":8123,"lensUser":"lensuser","lensPassword":"your-password"}
]'
...
> **Legacy:** `CLICKHOUSE_HOST`/`LENS_USER` etc. are deprecated — see [deprecated variables](#13-deprecated-variables) below.
...
services:
  clicklens:
    ...
    ports:
      - "3000:3000"
    environment:
      - CLICKHOUSE_CLUSTERS=[{"id":"primary","label":"Primary","host":"clickhouse","port":8123,"lensUser":"lensuser","lensPassword":"your-password"}]
...
- Verify `CLICKHOUSE_CLUSTERS` host and port values are correct
...
ClickLens supports connecting to multiple independent ClickHouse clusters from a single instance.

### 8.1. Configuration

Define a `CLICKHOUSE_CLUSTERS` JSON array with one entry per cluster:

```bash
CLICKHOUSE_CLUSTERS='[
  {"id":"prod","label":"Production","host":"clickhouse-prod.internal","port":8123,"lensUser":"lensuser","lensPassword":"password"},
  {"id":"staging","label":"Staging","host":"clickhouse-staging.internal","port":8123,"lensUser":"lensuser","lensPassword":"password"}
]'
```
...
| ------------------- | -------- | ------------------------------------------------------------------ |
| `id`                | yes      | Unique identifier (e.g. `"prod"`, `"staging"`)                     |
| `label`             | yes      | Display name shown in the login selector                           |
| `host`              | yes      | ClickHouse HTTP hostname                                           |
| `port`              | no       | Default: `8123` (or `8443` for HTTPS)                              |
| `lensUser`          | yes      | Service account for metadata queries on this cluster               |
| `lensPassword`      | yes      | Service account password                                           |
| `secure`            | no       | Use HTTPS for this cluster                                         |
| `verifySsl`         | no       | Verify SSL certificate (default: `true`)                           |
| `clickhouseCluster` | no       | Explicit ClickHouse cluster name override for `ON CLUSTER` queries |

### 8.2. Session Isolation

Each login session is pinned to a single cluster. All queries, monitoring, schema browsing, and access control operations target that cluster. Caches are isolated per cluster by `config.clusterId`.

### 8.3. Compatibility

- Single-cluster mode via legacy env vars (`CLICKHOUSE_HOST`/`LENS_USER`) is still supported but deprecated — a `console.warn` is printed once at startup. Migrate to `CLICKHOUSE_CLUSTERS`.
- A registry entry with `id: "default"` overrides legacy env vars
- The login page shows a cluster selector when multiple clusters are registered

### 8.4. Demo (Docker Compose)

A standalone demo compose file is available:
...
After changing init credentials, restart with fresh volumes:
