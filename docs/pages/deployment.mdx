...
| `LENS_USER` *(deprecated)*   | Service account username. Migrate to `lensUser` in `CLICKHOUSE_CLUSTERS`.                          | `lensuser`                                             |
| `LENS_PASSWORD` *(deprecated)* | Password for the Lens service account. Migrate to `lensPassword` in `CLICKHOUSE_CLUSTERS`.       |                                                        |
| `CLICKHOUSE_HOST` *(deprecated)* | ClickHouse server hostname. Migrate to `host` in `CLICKHOUSE_CLUSTERS`.                         | `clickhouse.example.com`                               |""`                                           | Password for the Lens service account |
| `SESSION_SECRET`  | Encryption key for session cookies (≥32 chars) | `your-32-character-secret-key...`     |
...
| `CLICKHOUSE_CLUSTERS`      | (empty)                         | JSON array of cluster definitions for multi-cluster mode. Each entry: `id`, `label`, `host`, `port`, `lensUser`, `lensPassword`. Replaces legacy `CLICKHOUSE_HOST`/`LENS_USER`.                                          |
...
> **Legacy:** `CLICKHOUSE_HOST`/`LENS_USER` etc. are deprecated — see [deprecated env vars](#deprecated) below.
...
Instead of the legacy single-cluster env vars, define a `CLICKHOUSE_CLUSTERS` JSON array:
...
- Single-cluster mode (`CLICKHOUSE_HOST`/`LENS_USER`) still works as before
- A registry entry with `id: "default"` overrides legacy env vars
- The login page auto-detects multi-cluster mode from the `CLICKHOUSE_CLUSTERS` env var
