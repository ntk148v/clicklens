...
When ClickLens is configured with multiple ClickHouse clusters, a cluster selector appears on the login page.

### 10.1. Login Flow

1. Select a cluster from the dropdown
2. Enter your ClickHouse credentials for that cluster
3. Click **Login**

All subsequent operations (queries, monitoring, schema browsing, access control) are pinned to the selected cluster. The **ClusterBadge** in the header shows which cluster you're connected to.

### 10.2. Isolation Guarantees

- Caches are keyed by `config.clusterId` — no data leaks between clusters
- The `CLICKHOUSE_CLUSTER` env var / config setting determines which ClickHouse cluster name is used for `ON CLUSTER` queries
- Each cluster has its own Lens service account for metadata operations
- Sessions are isolated per cluster
